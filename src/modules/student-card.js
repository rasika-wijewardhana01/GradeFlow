...
function renderStudentCard() {
  ...
  const waBtn = document.getElementById('scShareWABtn');
...
// ── WhatsApp share — current student card ──
function scShareWhatsApp() {
  const sel = document.getElementById('scStudentSelect');
  const idx = parseInt(sel?.value);
  ...
  const ex  = (document.getElementById('examLabel')    || {}).value?.trim() || 'Exam';

  const lines = [
    `📊 *Result Card — ${r.student}*`,
    cls ? `Class: ${cls} | ${ex}` : ex,
    '',
    `🎯 Overall: ${r.pct}% — Grade ${r.grade}`,
    `🏆 Rank: #${r.rank}`,
    '',
    '📚 Subject Breakdown:',
  ];
  ...
  (window.subjects || []).forEach(sub => {
    ...
    if (v === 'AB') { lines.push(`  ${sub.name}: AB (Absent)`); return; }
    const pct = sub.max > 0 ? Math.round((v / sub.max) * 100) : 0;
    lines.push(`  ${sub.name}: ${v}/${sub.max} (${pct}%)`);
  });
